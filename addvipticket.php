        ...
        <div class="mainmenu">
           ...
           <div class="right">
              ...
              <ul>
                ...
                <li><a class="active" href="addvipticket.php">VIP Ticket</a></li>
...
$connect=mysqli_connect("localhost","root","","park_sept17");
			...
			if(@$_POST['insert']){
					...
					if($_FILES['vip_ticket_image']){
						...
						if ((($_FILES["vip_ticket_image"]["type"] == "image/gif")
						 ...
						 && ($_FILES["vip_ticket_image"]["size"] < 50000000)){
							...
							$query="insert into vip_ticket
							values('','image/$vip_ticket_image','$vip_ticket_description','$vip_ticket_price','$vip_ticket_quantity')";
							$result= mysqli_query($connect,$query);
							if(!$result){
								echo "<center>Error: </center>".mysqli_error($connect);
							}
							else{
								echo "<center>VIP Ticket Data Inserted.</center>";
							}
				...
				if($_GET){
					...
					$vip_ticket_no=$_GET["vip_ticket_no"];
					$query="delete from vip_ticket where vip_ticket_no=$vip_ticket_no";
					$result= mysqli_query($connect,$query);
				if(!$result){
					echo "<center>Error: </center>".mysql_error();
				}
				else{
					echo "<center>VIP Ticket Data Deleted. </center>";	}}}?>
...
<html><head>
	...
	<body>
			...
			<form action="addvipticket.php" method="post" enctype="multipart/form-data">
			...
			<caption style="font-size:30px">VIP Ticket Information</caption>
					...
					<td><input type="file" id="vip_ticket_image" name="vip_ticket_image" /></td>
				</tr>
				<tr>
					<td><label>VIP Ticket Description:</label></td>
					<td><input type="text" name="vip_ticket_description"></td>
				</tr>
				<tr>
					<td><label>VIP Ticket Price:</label></td>
					<td><input type="text" name="vip_ticket_price"></td>
				</tr>
				<tr>
					<td><label>VIP Ticket Quantity:</label></td>
					<td><input type="number" name="vip_ticket_quantity" /></td>
				</tr>
				<tr> 
					<td colspan="2" align="middle"><input type="submit" value="Add VIP Ticket" name="insert" 
